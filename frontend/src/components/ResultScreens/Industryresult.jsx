    ...
    const promises = Object.entries(property_id).map(async ([key, value]) => {
      ...
      try {
        ...
        if (data) {
          ...
          console.log("Ushan::::", Essential_supply_vendor_lookup_df);
          ...
          if (Essential_supply_vendor_lookup_df["No_of_vendors_found"][key] && Array.isArray(Essential_supply_vendor_lookup_df["No_of_vendors_found"][key]) && Essential_supply_vendor_lookup_df["No_of_vendors_found"][key].length > 0) {
            ...
            console.log("supply_id:", ess_supply_id);
            console.log("No_of_vendors_found:", ess_No_of_vendors_found);
            console.log("Distance:", ess_Distance);
            ...
            ess_supply_id.forEach((supply, index) => {
              if (ess_No_of_vendors_found[index] !== undefined && ess_Distance[index] !== undefined) {
                ...
                console.warn("Skipping index:", index, "due to missing data");
              }
            });
          }
          ...
          if (
            nonEssential_supply_vendor_lookup_df["No_of_vendors_found"][key] &&
            Array.isArray(nonEssential_supply_vendor_lookup_df["No_of_vendors_found"][key]) &&
            nonEssential_supply_vendor_lookup_df["No_of_vendors_found"][key].length > 0
          ) {
            ...
            console.log("supply_id:", noness_supply_id);
            console.log("No_of_vendors_found:", noness_No_of_vendors_found);
            console.log("Distance:", noness_Distance);
          ...
          console.log("nonessential_supply_and_vendor", nonessential_supply_and_vendor);
          ...
          <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
            <li className="me-2">
              <button
                ...
                className={`inline-flex items-center justify-center p-4 border-b-2 rounded-t-lg group transition-all ${activeTab === "property"
                    ? "text-blue-600 border-blue-600 dark:text-blue-500 dark:border-blue-500"
                    : "border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300"
                  }`}
              >
                <FaBuilding
                  className={`w-5 h-5 me-2 ${activeTab === "property" ? "text-blue-600 dark:text-blue-500" : "text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-300"
                    }`}
                />
                Property
              </button>
            ...
            <li className="me-2">
              <button
                ...
                className={`inline-flex items-center justify-center p-4 border-b-2 rounded-t-lg group transition-all ${activeTab === "map"
                    ? "text-blue-600 border-blue-600 dark:text-blue-500 dark:border-blue-500"
                    : "border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300"
                  }`}
              >
                <FaMapMarkedAlt
                  className={`w-5 h-5 me-2 ${activeTab === "map" ? "text-blue-600 dark:text-blue-500" : "text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-300"
                    }`}
                />
                Map
              </button>
