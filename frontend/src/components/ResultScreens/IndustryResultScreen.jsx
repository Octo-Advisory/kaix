...
import { RiShipFill, RiSignalTowerFill } from "react-icons/ri";
...
const getVendorData = async (doc, records) => {
  if (!records) return [];
  console.log('Records passed........', records);
  try {
    const result = await call.get("frontend_app.Management_Class.helpers.utility.get_docs_with_children", {
      doctype: doc,
      names: JSON.stringify(records)
    });
    return result.message;
  } catch (err) {
    console.log("Error fetching child records:", err);
    setSomeError(true)
  }
};
    ...
    const fetchPropertyData = async (analytics_response) => {
      ...
      if(lastChatId){
        await updateDoc("Chat history", lastChatId, {
          result: JSON.stringify({"result":{"Analytics_response":updatedSolutions}}),
        ...
        {source==='SolutionScreen' && (<div className='sticky top-0 left-0 w-full h-fit flex flex-col z-99  bg-white border-b border-gray-300'>
          <div className='relative w-full h-12 p-4 flex flex-row justify-between mb-4'>
            <div className='relative flex flex-row gap-2 items-center'>
              <h1 className="text-2xl font-bold text-[#0e2044]"> {uiData?.main_title || "Industrial Property Solutions"}<span className="bg-gradient-to-br from-[#3CB35B] via-[#2E8C4A] to-[#152F4F] font-bold bg-clip-text text-transparent ml-2">{randomTitle}</span></h1>
            </div>
          ...
          {activeTab==='Property Details' && (<div className='relative w-full min-h-36 h-36 flex flex-row p-3 gap-4 overflow-x-auto overflow-y-hidden hide-scrollbar'>
            ...
            {solutions && solutions.map((data,index) =>{
              ...
              return (<div key={index} onClick={()=> {setSelectedProperty(solutions[index])}} className={`relative cursor-pointer min-h-28 h-28 min-w-[200px] w-[200px] border border-gray-400 flex flex-col rounded-md hover:cursor-pointer overflow-hidden transform transition duration-300  shadow-lg ${isSelected ? '-translate-y-1 border-2 border-gray-700': 'hover:shadow-lg hover:-translate-y-1 '}`} >
                <div className='relative min-h-8 h-8 flex flex-row px-2 items-center justify-end max-h-8 bg-gradient-to-r from-[#0e2044] to-[#41b655] w-full opacity-80'>
      ...
      {selectedProperty ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-8 w-full gap-4">
          ...
          <div className="card p-4 bg-white rounded shadow-md gap-4 flex flex-col items-start col-span-2">
            ...
            {/* <h2 className="text-base font-semibold text-primary flex flex-row gap-2 items-center">Location Map <div className="relative inline-block"><FaRegQuestionCircle size={15} className="text-gray-400" onMouseEnter={() => handleMouseEnter('tooltip1')} onMouseLeave={() => handleMouseLeave('tooltip1')}/><div className={`${visibleTooltips.tooltip1 ? 'block' : 'hidden'} absolute top-6 left-1/2 transform -translate-x-1/2 text-xs text-white bg-black rounded-md p-2 min-w-fit whitespace-nowrap z-[333]`}>Shows the exact location of the property/project on the map</div></div></h2> */}
              <span className="text-sm font-bold text-black flex flex-row items-center gap-2"><span className='h-6 w-6 p-2 rounded-full flex items-center font-semibold shadow-md justify-center border-2 border-black text-black bg-transparent'>{selectedProperty?.propertyIndex}</span> Property in {selectedProperty.area}</span>
            </div>
            ...
            <div className="text-xs text-gray-600 flex justify-between relative w-full flex-row">
              ...
              <button className="text-primary hover:underline flex flex-row items-center cursor-pointer" onClick={()=>{setSingleMapWindow(true)}} >View Full Map <IoIosArrowForward /></button>
            </div>
            ...
            <div className="flex justify-between items-center border-b border-gray-100">
              ...
              <div className="flex justify-between items-center border-b border-gray-100">
                <span className="text-sm text-gray-600">Business Location Type</span>
          ...
          <div className="card p-4 bg-white rounded  shadow-md gap-6 relative flex flex-col items-start col-span-3">
            <div className='relative flex flex-row items-center justify-between'>
              <div className="flex gap-2 items-center relative flex-row">
                <h2 className="text-base font-semibold text-primary relative">{uiData?.vendor_card_title || "Supply Chain Accessibility"}</h2>
                <div className="tooltip relative inline-block">
                  ...
                  <div className={`${visibleTooltips.tooltip2 ? 'block' : 'hidden'} absolute top-6 left-1/2 transform -translate-x-1/2 text-xs text-white bg-black rounded-md p-2 min-w-fit whitespace-nowrap z-[333]`}>{uiData?.vendor_tooltip ||"Nearby vendors sorted by distance and relevance to the selected property"}</div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-8 gap-4">
          ...
          <div className="card p-4 bg-white rounded shadow-md col-span-2 gap-6 flex flex-col items-start">
            <div className='relative gap-2 flex flex-row items-center'>
              <h2 className="text-base relative font-semibold text-primary">Location Intelligence Summary</h2>
              <div className="tooltip relative inline-block">
                ...
                <div className={`${visibleTooltips.tooltip4 ? 'block' : 'hidden'} absolute top-6 left-1/2 transform -translate-x-1/2 text-xs text-white bg-black rounded-md p-2 min-w-fit whitespace-nowrap z-[333]`}>Nearby Transport & Connectivity Distances</div>
              </div>
          ...
          <div className="w-full flex flex-col col-span-2 p-4 bg-white rounded shadow-md">
            <div className="flex flex-row justify-start self-start items-center w-full">
              <h2 className="text-base font-semibold text-primary flex flex-row gap-2 items-center">Decision Support Radar <div className='relative inline-block'><FaRegQuestionCircle size={15} className="text-gray-400" onMouseEnter={()=>{handleMouseEnter('tooltip7')}} onMouseLeave={()=>{handleMouseLeave('tooltip7')}}/><div className={`${visibleTooltips.tooltip7 ? 'block' : 'hidden'} absolute top-6 left-1/2 transform -translate-x-1/2 text-xs text-white bg-black rounded-md p-2 min-w-fit whitespace-nowrap z-[333]`}>Summarizes core property suitability factors.</div></div></h2>
            </div>
          ...
          <div className="card p-4 bg-white rounded col-span-4 pb-12 shadow-md gap-6 relative flex flex-col items-start">
            <div className="flex flex-row justify-between items-center w-full">
              <h2 className="text-base font-semibold text-primary flex flex-row gap-2 items-center"> Subsidy & Support Matrix <div className='relative inline-block'><FaRegQuestionCircle size={15} className="text-gray-400" onMouseEnter={()=>{handleMouseEnter('tooltip6')}} onMouseLeave={()=>{handleMouseLeave('tooltip6')}}/><div className={`${visibleTooltips.tooltip6 ? 'block' : 'hidden'} absolute top-6 left-1/2 transform -translate-x-1/2 text-xs text-white bg-black rounded-md p-2 min-w-fit whitespace-nowrap z-[333]`}>Applicable Government Incentives & Schemes</div></div> 
        ...
        <div className="card p-4 bg-white max-h-[350px] min-h-[350px] flex flex-col gap-2 h-full rounded shadow-md col-span-2">
          <div className="flex justify-between h-fit items-center mb-3">
            <h2 className="text-base font-semibold text-primary flex flex-row gap-2 items-center">Workforce Availability Insights <div className='relative inline-block'><FaRegQuestionCircle size={15} className="text-gray-400" onMouseEnter={()=>{handleMouseEnter('tooltip5')}} onMouseLeave={()=>{handleMouseLeave('tooltip5')}} />
          ...
          <div className={`relative col-span-6 w-full rounded p-4 max-h-[350px] bg-white transition-all shadow-md duration-300 overflow-hidden`}>
            <div className='h-fit w-full flex flex-row border-b  items-center justify-start gap-4 '>
              <div className='relative flex flex-col gap-1 w-fit bg-white cursor-pointer select-none' onClick={()=>{setMarketAndLaws('Local Laws'), setShowMarketTrends(false), setShowLocalLaws(true), handleLocalLawsClick()}}>
                <h1 className='font-semibold text-base text-black flex flex-row gap-2 items-center'><ImHammer2 className='text-blue-600'/> Local Laws & Taxes</h1>
              ...
              <div className='relative flex flex-col gap-1 w-fit bg-white cursor-pointer select-none' onClick={()=>{setMarketAndLaws('Market Trends'), setShowMarketTrends(true), setShowLocalLaws(false), handleLocalLawsClick()}}>
                <h1 className='font-semibold text-base text-black flex flex-row gap-2 items-center'><FaChartLine className='text-purple-600'/> Market Trends</h1>
