...
import ProgressScreen from '../ProgressScreen/ProgressScreen'; // Import the ProgressScreen component

function Chatscreen() {
  const [message, setMessage] = useState('');
  const [isProgressVisible, setIsProgressVisible] = useState(false); // State to show/hide ProgressScreen
  ...
  const handleConfirmation = (response) => {
    ...
    if (response === 'yes') {
      setIsProgressVisible(true); // Show the progress screen when the user clicks "Yes"
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col items-center">
      <Navbar />
      <div className={`flex-1 overflow-y-auto p-4 flex justify-center w-full chatscreen ${isProgressVisible ? 'blurred' : ''}`} ref={ref}>
        {messages.length > 0 ? (
            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex gap-5 justify-start`}>
                {msg.sender !== 'user' ? (
                  ...
                  <img src={userIcon} alt="" className="h-8 w-8 relative rounded-full" />
                )}
          ...
          <div className="flex justify-center items-center"><p className="text-5xl text-[#242f6a]">What Can I Help You?</p></div>
        )}
      </div>
      ...
      {confirmationPending && (
        <div className="confirmation-box p-4 rounded-lg w-[45%] flex flex-col items-center">
          ...
          <div className="flex space-x-4 mt-2">
            <button className="bg-green-500 text-white px-4 py-2 rounded" onClick={() => handleConfirmation('yes')}>
              Yes
            </button>
            <button className="bg-red-500 text-white px-4 py-2 rounded" onClick={() => handleConfirmation('no')}>
              No
            </button>
          </div>
        </div>
      ...
      {isProgressVisible && <ProgressScreen />} {/* Render the ProgressScreen */}
