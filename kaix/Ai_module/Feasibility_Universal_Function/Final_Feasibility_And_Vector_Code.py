...
from kaix.Ai_module.Feasibility_Universal_Function.Final_Universal_Function import (
...
base_dir = os.path.expanduser("~")
PERSIST_ROOT = os.path.join(base_dir, "frappe-bench/apps/kaix/kaix/vectors")
...
class AdvancedRAGSystem:
    ...
    def create_advanced_retriever(self, vector_store:dict|str, status:bool, doctype:str):
        ...
        if root:
            ...
            # 2) Pull texts+metadatas from the store (you can page if large)
            #    If you have many docs, iterate with limit/offset to avoid RAM spikes.
            # all_ids = vs.get()["ids"]
            # docs, metas = vs.get(ids=all_ids)["documents"], vs.get(ids=all_ids)["metadatas"]
            # documents = [Document(page_content=t, metadata=m) for t, m in zip(docs, metas)]
            # 1) Guard empty collection early
            try:
...
def process_feasibility_report(vector_store, doctype, status):    
    ...
    #         break
    #     # response = rag.generate_answer(user_question, file_path)
    #     response = rag.generate_answer(user_question)
    #     keyword = rag.extract_keyword(user_question, response['verified_answer'])
    #     display(Markdown(f"**Answer:** {response['verified_answer']}"))
    #     display(Markdown(f"**Extracted Keyword:** {keyword}"))
...
# def query_classification(status:bool, llm_model="meta-llama/llama-4-maverick-17b-128e-instruct", temperature=0.5):
    ...
    doc_info, questions_statement, questions_list = final_call(vector_store=vector_store, doctype=doctype, status=status)
