...
decider_prompt = PromptTemplate(
    ...
    1. **Hierarchical Analysis**:
       - Analyze the search results to count unique data points for each level:
         ...
         - **Country Level**: Count data points mentioning 'Cambodia' or synonyms (e.g., 'Cambodian') relevant to {sub_sector}/{segment} (e.g., national market size, production).
         - **Global Level**: Count global data points relevant to {sub_sector}/{segment} in Cambodia (e.g., technological advancements).
       - Use fuzzy matching (case-insensitive, partial matches) for city/state synonyms.
       - A data point is unique if it provides a distinct metric, entity, or qualitative insight (e.g., '70% production' and '$500 million investment' are separate).
...
summarize_prompt = PromptTemplate(
    ...
    1. **Hierarchical Analysis of Search Results**:
       - Analyze search results in this order: city, state, country, global, to extract all relevant information:
         ...
         - **State Level**: Identify all data points explicitly mentioning '{state}' or synonyms (e.g., 'Svay Rieng') with segment-specific metrics, entities, or qualitative insights.
         - **Country Level**: Identify all data points specific to Cambodia with segment-specific metrics, entities, or qualitative insights.
         - **Global Level**: Identify global data points relevant to {segment} in Cambodia (e.g., global market trends, technological advancements).
       - Use fuzzy matching (case-insensitive, partial matches) for city/state synonyms.
    ...
    2. **Trend Identification**:
       ...
       - For each allocated trend:
         - **Bullet Points**: Create 2–5 bullet points (10–15 words each) that maximize the use of relevant data points from the assigned scope (city, state, country, or global). Include specific metrics (e.g., **70% production**), entities (e.g., **VinFast**), or qualitative insights with local context (e.g., 'Svay Rieng's EV hubs expand production').
         - **Title**: Craft a title that accurately summarizes the bullet points’ theme (e.g., investment, production, policy) and scope. The title must only mention the location (city, state, Cambodia, or Globally) if all bullet points pertain to that scope. Avoid generic or misleading titles.
       ...
       - **Uniqueness**: Ensure each trend has a distinct theme (e.g., investment, production, policy, innovation, socio-economic impact), metric, and entity (e.g., avoid repeating 'growth' or 'VinFast' unless distinctly supported by different excerpts).
       - **Scope Alignment**: Ensure all bullet points exactly match the trend’s scope (e.g., state-level for 'In Svay Rieng'). Reject bullets that deviate (e.g., Cambodia-level data in a state trend).
    ...
    3. **Relevance and Exclusion**:
       ...
       - Exclude trends unrelated to {sub_sector}/{segment} (e.g., reject 'textiles' for EV manufacturing) or geographically irrelevant (e.g., North America trends for Svay Rieng).
    ...
    4. **City/State Fallback**:
       - If city-level data is absent (e.g., no Bavet-specific metrics), generate qualitative city-level trends only if supported by state-level results, using specific entities and local context (e.g., 'GSM expands in Bavet, based on Svay Rieng's EV growth'). Log internally 'Limited city data, based on state context', but do not include in output.
    ...
    5. **Summary Format**:
       ...
       - Use plain text (no bullet points or bolding) to describe each trend, including metrics, entities, scope (e.g., 'In Svay Rieng'), and theme (e.g., production).
       - Do not mention sources or phrases like 'from', 'as mentioned', or 'according to' in the summary or output.
       - Log internally the theme, scope, and supporting excerpt for each trend (e.g., 'Positive: Svay Rieng's EV production, Theme: Production, Scope: State').
...
format_prompt = PromptTemplate(
    ...
    2. **Title Requirements**:
       - **Header Format**: Use exactly `## **Title**` for all titles (e.g., ## **In Svay Rieng, EV Sector Ignites 70% Production**). Do not use `###`, `#`, or unbolded titles.
       - **Scope Accuracy**: Titles must exactly reflect the scope of the bullet points:
         - Use 'In {city}' only if all bullets are city-specific (e.g., 'In Bavet' for Bavet-based metrics).
         - Use 'In {state}' only if all bullets are state-specific (e.g., 'In Svay Rieng' for Svay Rieng's production).
         - Use 'In Cambodia' for country-level bullets (e.g., 'In Cambodia' for 377,000 units production).
         - Use 'Globally' for global bullets relevant to {segment} in Cambodia (e.g., 'Globally' for EV innovation).
         - Validate that all bullets match the title’s scope; reject titles if any bullet deviates (e.g., no 'In Svay Rieng' if bullets are Cambodia-level).
       - **Theme and Summarization**: Highlight the trend’s theme (e.g., production, investment, policy) and include one key metric (e.g., **70% production**) or entity (e.g., **VinFast**) as a teaser. Ensure themes are distinct (e.g., no two 'growth' trends).
       - **Engagement**: Use vivid verbs for positive trends (e.g., 'Ignites', 'Pioneers') and question-provoking phrases for negative trends (e.g., 'Can Cambodia Overcome Charging Constraints?').
    ...
    3. **Bullet Point Requirements**:
       - Positive trends: 2–5 bullets (10–15 words), expanding the Ascending order: 1
         - **Svay Rieng** accounts for **70%** of Cambodia's EV two-wheeler production.
         - **$500 million** investment in EV manufacturing by 2025 creates **15,000 jobs**.
         - **VinFast** and **GSM** establish Svay Rieng as an EV hub.
       ...
       - Each bullet must include at least one bolded **metric** or **entity** (e.g., **70% production**, **VinFast**).
    ...
    4. **Relevance Check**:
       ...
       - Exclude unrelated trends (e.g., reject textiles for EV manufacturing) or geographically irrelevant trends (e.g., North America trends for Svay Rieng).
...
fallback_prompt = PromptTemplate(
    ...
    template="""
    No market trends data found for {main_industry} ({sub_sector}, {segment}) in {city}, {state} for 2024-2025. Provide exactly 5 plausible trends based on general knowledge for {sub_sector} in {state} if feasible, else Cambodia, following these detailed instructions, with strict Markdown formatting, exact scope alignment with bullet points, and without mentioning sources or phrases like 'from', 'as mentioned', or 'according to':

    1. **Hierarchical Analysis**:
       - Prefer {state}-level trends if general knowledge supports (e.g., Svay Rieng's EV manufacturing).
       - Fall back to Cambodia if state data is insufficient, labeling as 'In Cambodia'.
       - Use 'Globally' for global trends relevant to {segment} in Cambodia (e.g., global EV innovation).
       - For city-level trends, use qualitative insights with specific entities (e.g., 'GSM expands in Bavet') only if supported by state-level context.
       - Log internally the number of data points and supporting context for each level.

    2. **Dynamic Trend Allocation**:
       - Allocate exactly 5 trends, prioritizing state-level (e.g., Svay Rieng) over country and global based on general knowledge:
